...
export const PhotoWithPhotoAndCaption = ({ id, post, user }) => {
  ...
  const [scopeMoreOption, setScopeMoreOption] = useState();
  const ueId = {
    userId: user.id,
    emoteId: 1,
    likableId: post.id,
  ...
  useEffect(() => {
    ...
    if (elementScope.className === "anchor-hover-like-hover" && !likeHover)
  ...
  const handleOnClickEmoteButton = (e, type) => {
    ...
    if (clickedLike === false) {
      ...
      if (type === 1) {
        setEmoteId(1);
        e.target.parentNode.parentNode.previousSibling.firstChild.lastChild.innerHTML =
          "Like";
        e.target.parentNode.parentNode.previousSibling.firstChild.firstChild.src =
      ...
      .then((data) => {
        for (const [key, value] of Object.entries(data)) {
          if (comments.length !== 0) {
            if (
              comments[parseInt(key)].id &&
              !comments[parseInt(key)].id === value.id
            )
  ...
  const handleCommentOnClick = (e) => {
    ...
    const commentsInPost =
      e.target.parentNode.parentNode.nextElementSibling.children;
    ...
    if (!commentOpen) {
      ...
      e.target.parentNode.parentNode.nextElementSibling.style.padding =
        "6px 16px 16px 16px";
      ...
      if (comments.length === 0) {
        csrfFetch(`/api/comments/${post.id}`)
          ...
          .then((data) => {
            for (const [key, value] of Object.entries(data)) {
              if (comments.length !== 0) {
                if (!comments[parseInt(key)].id === value.id)
                  setComments((comments) => [value, ...comments]);
              } else {
                setComments((comments) => [value, ...comments]);
              }
            }
  ...
  useEffect(() => {
    if (editPostValue.length > 0) {
      document.getElementsByClassName("save-button-place-post")[0].style.color =
        "white";
      document.getElementsByClassName(
        "save-button-place-post"
      )[0].style.backgroundColor = "#0A66C2";
      document.getElementsByClassName(
        "placeholder-post-edit-form"
  ...
  document.addEventListener("mousedown", (event) => {
    ...
    if (event.target.className !== "delete-comment-from-post") {
      ...
      setMoreOptionPost(false);
    }
  });

  const handleOnClickMoreOptionPost = (e) => {
    // console.log(post.userId, user.session.user.id)
    setScopeMoreOption(e.target);
    ...
    if (e.target.className.baseVal === "svg-more-option-post-d") {
      moreOptionPost
        ? (e.target.parentNode.parentNode.nextElementSibling.style.display =
            "none")
        : (e.target.parentNode.parentNode.nextElementSibling.style.display =
            "flex");
    }
  ...
  const handleOnClickEditPost = (e) => {
    document.getElementsByClassName("text-area-for-edit-post")[0].innerText =
      post.body ? post.body : "";
    document.getElementsByClassName("edit-form-post-form")[0].style.display =
      "flex";
  ...
  const handleClickSubmitEditForm = (e) => {
    ...
    csrfFetch(`/api/posts/${post.id}`, {
      ...
      e.target.parentNode.parentNode.parentNode.parentNode.parentNode.firstChild.children[1].children[3].firstChild.innerText =
        editPostValue;
    ...
    <BrowserRouter>
        ...
        <div className="pop-up-edit-form-for-post">
          <div className="heading-top-edit-form">
            ...
            <div className="svg-width-to-right">
              <svg
                className="close-edit-form-post"
              ...
              <div className="width-setter-context-box">
                <div className="context-edit-post-text-area">
                  ...
                  <div
                    ...
                    onInput={(e) => setEditPostValue(e.target.innerText)}
                    className="text-area-for-edit-post"
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div className="bottom-nav-bar-submit-other-options">
            <button
              onMouseLeave={handleUnhoverSaveButton}
              onMouseEnter={handleHoverSaveButton}
              onClick={handleClickSubmitEditForm}
              className="save-button-place-post"
            >
              Save
            </button>
          </div>
      ...
      <div className="post-with-photo-and-caption">
        <div className="more-options-for-post">
          <svg
            className="svg-more-option-post"
            onClick={handleOnClickMoreOptionPost}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            data-supported-dps="24x24"
          ...
          >
            <path
              className="svg-more-option-post-d"
          ...
          {post.userId === user.id ? (
            <>
              <button
                ...
                className="delete-comment-from-post"
              >
                <img
                  alt="Trash icon"
              ...
              >
                <img
                  alt="Edit icon"
                  src="https://img.icons8.com/sf-ultralight-filled/512/pencil.png"
                ></img>
                Edit
              </button>
            </>
            ...
            <>
              <button className="delete-comment-from-post">
                <img
                  alt="Edit icon"
                  src="https://img.icons8.com/material-rounded/512/flag.png "
                ></img>
                Report
              </button>
            </>
        ...
        <div className="caption-of-post">
          <div className="body-caption-post-text">{body}</div>
        </div>
        ...
        <div className="like-comment-post-association">
          <div className="like-icons-active">
            {/* append icon svg */}
            {topLikeArray.includes(1) && (
              <img
                className="emote-mini"
                src="https://static-exp1.licdn.com/sc/h/8ekq8gho1ruaf8i7f86vd1ftt"
                alt="like"
              />
            )}
            {topLikeArray.includes(2) && (
              <img
                className="emote-mini"
            ...
            {topLikeArray.includes(7) && (
              <img
                className="emote-mini"
                src="https://static.licdn.com/sc/h/4mv33903v0o9ikpwfuy2ftcc6"
                alt="like"
              />
            )}
        ...
        <div className="bottom-button-comment-and-like">
          <div
            ...
            onClick={(e) => handleOnClickLikeButton(e, 1)}
            onMouseEnter={onHoverLikeButton}
            onMouseLeave={onUnHoverLikeButton}
          >
            {!clickedLike && (
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            {clickedLike && (
              <>
                {emoteId === 1 && (
                  <img
                    className="like-emote-button-mini"
                    onClick={(e) => handleOnClickEmoteButton(e, 1)}
                ...
                {emoteId === 2 && (
                  <img
                    className="like-emote-button-mini-celebrate"
                    onClick={(e) => handleOnClickEmoteButton(e, 2)}
                ...
                {emoteId === 3 && (
                  <img
                    className="like-emote-button-mini"
                    onClick={(e) => handleOnClickEmoteButton(e, 3)}
                ...
                {emoteId === 4 && (
                  <img
                    className="like-emote-button-mini"
                    onClick={(e) => handleOnClickEmoteButton(e, 4)}
                    src="https://static-exp1.licdn.com/sc/h/ktcgulanbxpl0foz1uckibdl"
                    alt="funny"
                  />
                )}
                {emoteId === 5 && (
                  <img
                    className="like-emote-button-mini"
                    onClick={(e) => handleOnClickEmoteButton(e, 5)}
                ...
                {emoteId === 6 && (
                  <img
                    className="like-emote-button-mini"
                    onClick={(e) => handleOnClickEmoteButton(e, 6)}
                ...
                {emoteId === 7 && (
                  <img
                    className="like-emote-button-mini"
                    onClick={(e) => handleOnClickEmoteButton(e, 7)}
            ...
            <span className="name-of-type-of-emote">Like</span>
          </div>
        ...
        <div className="anchor-hover-like-hover">
          <div
            className="hover-like-option-choose-emote"
            onMouseEnter={(e) => {
              setLikeHover(true);
          ...
          >
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 1)}
              className="like-emote"
              src="https://static-exp1.licdn.com/sc/h/f4ly07ldn7194ciimghrumv3l"
              alt="like"
            />
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 2)}
              className="celebrate-emote"
              src="https://static-exp1.licdn.com/sc/h/3c4dl0u9dy2zjlon6tf5jxlqo"
              alt="celebrate"
            />
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 3)}
              className="support-emote"
              src="https://static-exp1.licdn.com/sc/h/9whrgl1hq2kfxjqr9gqwoqrdi"
              alt="support"
            />
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 4)}
              className="funny-emote"
              src="https://static-exp1.licdn.com/sc/h/ktcgulanbxpl0foz1uckibdl"
              alt="funny"
            />
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 5)}
              className="love-emote"
              src="https://static-exp1.licdn.com/sc/h/asmf650x603bcwgefb4heo6bm"
              alt="love"
            />
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 6)}
              className="insightful-emote"
              src="https://static-exp1.licdn.com/sc/h/39axkb4qe8q95ieljrhqhkxvl"
              alt="insightful"
            />
            <img
              onClick={(e) => handleOnClickEmoteButton(e, 7)}
              className="curious-emote"
              src="https://static-exp1.licdn.com/sc/h/1z80ze8ler6arc76a8rxsgqbh"
              alt="curious"
            />
          </div>
      ...
      <div className="comment-section-for-post-div">
        <div className="comment-bar-section-div">
          <div className="user-profile-picture">
            <div className="picture-crop-photo-div-smaller-post-c">
              <div className="crop-smaller-post-pfp-c">
                <img
                  className="author-post-pfp-post-c"
                  src={require("../../../../../assets/post/happy-businessman-isolated-handsome-man-260nw-609414131.png")}
                />
              </div>
            </div>
          ...
          <div className="comment-input-area">
            <form
              className="form-input-comment-send-post"
              onSubmit={handleOnSubmitNewComment}
            >
              <input
                className="input-comment-make-new-comment"
                onChange={(e) => setCommentInput(e.target.value)}
                value={commentInput}
                placeholder={"Add a comment..."}
              />
              <button
                className="submit-type-button-comment-form-make"
                type="submit"
              >
                Post
              </button>
